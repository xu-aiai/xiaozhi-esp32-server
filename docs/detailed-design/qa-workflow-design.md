...
4. 如果没有工具调用，则先收集完整回复文本
5. 服务端会对完整回复做一次回复语言校验，必要时触发语言校正
6. 校验后的完整文本再一次性写入 TTS 队列

补充说明：

- 当前主聊天回复已经不是“边流式生成边流式播报”
- 现在会先等 LLM 生成完整回复，再进入 TTS
- 这样做的原因是为了更强地保证“中文提问中文回答，英文提问英文回答”
...
| `回复语言检测结果` | 回复校验阶段 | 主 LLM 生成完整文本后、进入 TTS 前 | 校验回复语言是否与本轮目标语言一致 |
| `回复语言校正结果` | 回复校验阶段 | 回复语言不一致，进行校正后 | 记录校正后的语言检测结果 |
...
- 主聊天回复为了做语言一致性校验，已经改成“完整生成后再进 TTS”，首包延迟会高于早期的边生成边播模式
